...
class terminalPanel ( wx.Panel ):
	...
	def __init__( self, parent, transactionButtonName, uid ):
		...
		self.papa.SetFont( wx.Font( 14, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD, False, "Arial" ) )
		self.papa.SetForegroundColour( (0,0,0) )
	...
	def imposeDiscount (self, event):
		v = self.discountTC.GetValue()
		if v == "":
			v = 0
		self.billAfterDiscount.SetLabel( str( self.t.getCart().computeTotalBill() - int( v ) ))
		event.Skip()
	...
	def hideSearchSuggestion (self, event):
		if (self.suggestionList.HasFocus() == False):
			self.suggestionList.Hide()
			self.customerHeading.Show() # Show all cust info
			self.customerName.Show()
			self.customerContact.Show()
			self.newCustomerButton.Show()
			self.Layout()
		
	def search (self, event):
		self.suggestionList.SetItems( self.suggestionCandidatesAsList( self.inputTC.GetValue() ) )
		self.suggestionList.Show()
		self.bSizerTopSecondRowCustInfo.ShowItems(show=False) # Hide all cust info
	...
	def cartChange (self, event):
		...
		if (c == 2):
			...
			# if quantity was made zero
			if int(self.productsGrid.GetCellValue(r, 2)) == 0:
				self.t.removeFromCart( int(self.productsGrid.GetCellValue(r, 0)) )
				self.dumpCartInDvlc()
				return
		...
		if (c == 4):
			...
			self.t.getCartProducts()[r].price = self.t.getCartProducts()[r].origPrice - ( int(self.productsGrid.GetCellValue(r, 4)) / self.t.getCartProducts()[r].qty )
		...
		self.totalBill.SetLabel( "Total:  " + str( self.t.getCart().computeTotalBill() ))
		#self.discountTC.SetLabel( "Discount:  " + str( self.t.getCart().computeTotalDiscount() ))
		self.billAfterDiscount.SetLabel( str( self.t.getCart().computeTotalBill() - int( self.discountTC.GetValue() ) ))
	...
	def CheckOutFuncMajor (self, event):
		self.clearCartGrid()
		self.CheckOutFunc(event)
		self.billAfterDiscount.SetLabel( "0" )
	...
	def dumpCartInDvlc (self):
		...
		self.totalBill.SetLabel( "Total:  " + str( self.t.getCart().computeTotalBill() ))
		#self.discountTC.SetValue( "Discount:  " + str( self.t.getCart().computeTotalDiscount() ))
		self.billAfterDiscount.SetLabel( str( self.t.getCart().computeTotalBill() - int( self.discountTC.GetValue() ) ))

	def clearCartGrid (self):
		self.totalBill.SetLabel('Total:   0000000')
		self.discountTC.SetValue( "0" )
		self.billAfterDiscount.SetLabel( "0" )
