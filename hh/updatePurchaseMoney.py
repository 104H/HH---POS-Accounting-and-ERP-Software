...
class GetData(wx.Dialog):
	...
	def returnPurchase (self, purchaseId):
		qry = 'UPDATE purchase SET returned=1 WHERE id = %s' % (purchaseId)
		curs = conn.cursor()
		curs.execute(qry)
		
		qry = 'UPDATE currentinventory ci JOIN productpurchase pp ON ci.productId = pp.product SET ci.quantity = ci.quantity-pp.quantity WHERE pp.purchaseId = %s' % (purchaseId)
		curs.execute(qry)
		conn.commit()
