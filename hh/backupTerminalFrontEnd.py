...
class terminalPanel(wx.Panel):
    def __init__(self, parent, transactionButtonName, uid):
        ...
        y = searchSize[1] + y
        '''
        self.suggestionList = wx.ListBox(self.papa, size=(700, 120),
                                         choices=self.suggestionCandidatesAsList(self.inputTC.GetValue()), pos=(50, 60))
        ...
        self.customerHeading = wx.StaticText(self.papa, wx.ID_ANY, u"Customer Information", wx.DefaultPosition,
                                             wx.DefaultSize, 0)
        self.customerName = wx.StaticText(self.papa, wx.ID_ANY, u"Name:   ", wx.DefaultPosition, wx.DefaultSize, 0)
        self.customerContact = wx.StaticText(self.papa, wx.ID_ANY, u"Contact:   ", wx.DefaultPosition, wx.DefaultSize,
                                             0)
        # self.customerBalance = wx.StaticText( self.papa, wx.ID_ANY, u"Balance:   ", wx.DefaultPosition, wx.DefaultSize, 0 )
        self.newCustomerButton = wx.Button(self.papa, wx.ID_ANY, u"Select Customer", wx.DefaultPosition, wx.DefaultSize,
                                           0)
        ...
        self.totalBill.SetFont(
            wx.Font(18, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD, False, "Arial"))
        ...
        self.transactionButton = wx.Button(self.papa, wx.ID_ANY, transactionButtonName, wx.DefaultPosition,
                                           wx.DefaultSize, 0)
        ...
        self.transactionButton.SetFont(
            wx.Font(18, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD, False, "Arial"))
    ...
    def cartChange(self, event):
        ...
        if (c == 2):
            ...
            x = self.t.increaseQty(int(self.productsGrid.GetCellValue(r, 0)),
                                   int(self.productsGrid.GetCellValue(r, 2)) - self.t.getCartProducts()[r].qty)
            if x == True:
                self.productsGrid.SetCellValue(r, 5,
                                               str(self.t.getCartProducts()[r].qty * self.t.getCartProducts()[r].price))
                self.productsGrid.SetCellValue(r, 4, str(self.t.getCartProducts()[r].qty * (
                self.t.getCartProducts()[r].origPrice - self.t.getCartProducts()[r].price)))
            if type(x) == type(int()):
        ...
        if (c == 4):
            ...
            self.t.getCartProducts()[r].price = self.t.getCartProducts()[r].price - (
            int(self.productsGrid.GetCellValue(r, 4)) / self.t.getCartProducts()[r].qty)
        ...
        if (c == 5):
            ...
            self.t.getCartProducts()[r].price = int(self.productsGrid.GetCellValue(r, 5)) / self.t.getCartProducts()[
                r].qty

            self.productsGrid.SetCellValue(r, 4, str(
                (self.t.getCartProducts()[r].origPrice - self.t.getCartProducts()[r].price) * self.t.getCartProducts()[
                    r].qty))
            self.productsGrid.SetCellValue(r, 3, str(self.t.getCartProducts()[r].price))

        self.totalBill.SetLabel(
            "Total:  " + str(self.t.getCart().computeTotalBill() + self.t.getCart().computeTotalDiscount()))
    ...
    def suggestionCandidatesAsList(self, searchString):
        qry = 'SELECT DISTINCT products.name, products.codeName, products.costPrice, products.sellingPrice, products.barcode FROM `currentinventory`, `products` WHERE products.id = currentinventory.productId AND codeName LIKE "%' + str(
            searchString) + '%" OR BARCODE LIKE "%' + str(searchString) + '%"'
        conn = connectToDB()
        curs = conn.cursor()
        curs.execute(qry)
        r = curs.fetchall()
        sug = []
        for x in r:
            prod = x['codeName'] + "    - " + x['name'] + "   Rs. " + str(x['costPrice']) + "   Rs. " + str(
                x['sellingPrice']) + "   " + str(x['barcode'])
            sug.append(prod)
    ...
    def dumpCartInDvlc(self):
        for x in range(self.t.numberOfItems()):
            ...
            self.productsGrid.SetCellValue(x, 4, str(
                self.t.getCartProducts()[x].origPrice - self.t.getCartProducts()[x].price))
            self.productsGrid.SetCellValue(x, 5,
                                           str(self.t.getCartProducts()[x].qty * self.t.getCartProducts()[x].price))
        self.totalBill.SetLabel(
            "Total:  " + str(self.t.getCart().computeTotalBill() + self.t.getCart().computeTotalDiscount()))
        self.discountTC.SetLabel("Discount:  " + str(self.t.getCart().computeTotalDiscount()))
        self.billAfterDiscount.SetLabel("After Discount:  " + str(self.t.getCart().computeTotalBill()))
