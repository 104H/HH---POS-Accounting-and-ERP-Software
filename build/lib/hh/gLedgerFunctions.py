...
def invoiceReturnEntry (invoiceId):
	invoiceId = "Inv-" + str(invoiceId)
	
	qry = 'INSERT INTO generalLedger (headOfAc, transactionType, Debit, Credit) SELECT IF (headOfAc = 4, 23, headOfAc), "ReturnInvoice-%s", Credit, Debit FROM generalLedger WHERE transactionType = "%s" ORDER BY id DESC' % (invoiceId, invoiceId)
...
def manualEntry (hoaCredit, hoaDebit, transcType, cheque, amount):
	curs = conn.cursor()
	
	qry = 'INSERT INTO generalLedger (headOfAc, transactionType, chequeNo, Debit, Credit) VALUES ("%s", "%s", "%s", "%s", "0")' % (hoaDebit, transcType, cheque, amount)
	curs.execute(qry)
	
	qry = 'INSERT INTO generalLedger (headOfAc, transactionType, chequeNo, Debit, Credit) VALUES ("%s", "%s", "%s", "0", "%s")' % (hoaCredit, transcType, cheque, amount)
	curs.execute(qry)
